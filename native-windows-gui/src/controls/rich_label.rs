...
const NOT_BOUND: &str = "RichLabel is not yet bound to a winapi object";
const BAD_HANDLE: &str = "INTERNAL ERROR: RichLabel handle is not HWND!";
...
impl<'a> RichLabelBuilder<'a> {
    ...
    pub fn build(self, out: &mut RichLabel) -> Result<(), NwgError> {
        ...
        *out.line_height.borrow_mut() = self.line_height;
        ...
        } else if let Ok(color) = wh::get_background_color(parent.hwnd().unwrap()) {
            out.set_background_color(color);
        }
