    ...
    if handle.is_null() {
        ...
        if code == 2 && !strict {
            ...
            handle = match image_type {
                ...
                IMAGE_BITMAP => {
                    let dr = 32754_usize as *const u16;
                    LoadImageW(
                        ptr::null_mut(),
                        dr,
                        IMAGE_BITMAP,
...
pub unsafe fn bitmap_from_memory(source: &[u8]) -> Result<HANDLE, NwgError> {
    ...
    use winapi::um::wingdi::{
        CreateCompatibleBitmap, CreateCompatibleDC, SetDIBits, BITMAPFILEHEADER, BITMAPINFO,
